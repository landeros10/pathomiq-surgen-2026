#!/usr/bin/env bash
# Successive Halving — Round 3 launcher.
#
# Accepts MLflow run names from Round 2 (paste directly from MLflow UI),
# derives config paths, and relaunches training with Round 3 settings.
#
# Usage:
#   nohup ./scripts/run_round3.sh \
#       mmr-surgen-s1-all-round_2 \
#       > logs/stability/round3_orchestrator.log 2>&1 &
...
if [ "$#" -eq 0 ]; then
    ...
    echo "  Example: $0 mmr-surgen-s1-all-round_2"
    exit 1
fi
...
TOTAL=${#CONFIGS[@]}
echo "Round 3  --max-epochs ${MAX_EPOCHS}  --patience ${PATIENCE}  --run-suffix ${RUN_SUFFIX}"
...
for i in "${!CONFIGS[@]}"; do
    cfg="${CONFIGS[$i]}"
    stem="${STEMS[$i]}"
    log="logs/stability/${stem}-round_3.log"
    run_num=$((i + 1))
...
echo ""
echo "Round 3 complete — ${PASS} PASS  ${FAIL} FAIL  (${TOTAL} total)"
